...
;; from scheme/guile to common lisp.
...
(define (samesign x y)
  (or
   (= 0 x y) ;; returns #t if all areguments are numerically equal
   (= x y)   ;; #t if they are equal
   (and      ;; sign is n / abs n as long as n <> 0
    (not (= 0 x))
    (not (= 0 y))
    (= (/ x (abs x)) (/ y (abs y))))))
(samesign 0 0)   ==> #t
...
(define (classify-sentence-2 sent)
  (cond
   ((member (car sent) question-words) 'question)
   ((member 'was sent) 'passive)
   ((member 'by sent) 'passive)
   (#t 'active)))
...
;;      argument might not be evaluated. I'm not seeing a
;;      way to avoid evaluating an argument more than
;;      once given the knowledge we have at this point.
;;
;;      And I can't avoid evaluating the second argument
;;      even when we should not.
;;      (or #t (/ 3 0)) returns 3 while
;;      (my-or #t (/ 3 0)) throws an exception
...
;;      two arguments. To properly do an and, once a
;;      condition has been found false (#f or nil) no
;;      subsequent conditions are checked. This is hard
;;      to test with the tools we have so far, but the
;;      display function is good enough for now.
(define (my-and x y)
  (cond
   (x y)
   (#t #f)))
...
(define (successor val items)
  (cond
   ((member val items)
    (cond
     ((not (equal? '() (cdr (member val items)))) (car (cdr (member val items))))
     (#t 'no-successor)))
   (#t 'not-there)))
...
(define (addit val items)
  (cond
   ((member val items) 'found)
   (#t (cond ((equal? '() val) items)
             (#t (append items (list val)))))))
(addit 'a '(a b c)) ==> found
...
;;      two arguments and does the folloiwng.
